...
deps_frontend:
	go get -u github.com/sartura/drone-ui/dist
...
gen_template:
	go generate github.com/sartura/drone/server/template

gen_migrations:
	go generate github.com/sartura/drone/store/datastore/ddl
...
test_mysql:
	DATABASE_DRIVER="mysql" DATABASE_CONFIG="root@tcp(127.0.0.1:3306)/test?parseTime=true" go test github.com/sartura/drone/store/datastore

# docker run --publish=5432:5432 postgres:9.4.5
test_postgres:
	DATABASE_DRIVER="postgres" DATABASE_CONFIG="host=127.0.0.1 user=postgres dbname=postgres sslmode=disable" go test github.com/sartura/drone/store/datastore
...
build_static:
	go install -ldflags '${EXTLDFLAGS}-X github.com/sartura/drone/version.VersionDev=$(DRONE_BUILD_NUMBER)' github.com/drone/drone/drone
	mkdir -p release
	cp $(GOPATH)/bin/drone release/

# TODO this is getting moved to a shell script, do not alter
build_cross:
	GOOS=linux   GOARCH=amd64 CGO_ENABLED=0 go build -o release/linux/amd64/drone   github.com/sartura/drone/drone
	GOOS=linux   GOARCH=arm64 CGO_ENABLED=0 go build -o release/linux/arm64/drone   github.com/sartura/drone/drone
	GOOS=linux   GOARCH=arm   CGO_ENABLED=0 go build -o release/linux/arm/drone     github.com/sartura/drone/drone
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o release/windows/amd64/drone github.com/sartura/drone/drone
	GOOS=darwin  GOARCH=amd64 CGO_ENABLED=0 go build -o release/darwin/amd64/drone  github.com/sartura/drone/drone
